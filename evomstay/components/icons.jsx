// evomstay — icons + logo + small shared primitives
// Exposes: Logo, LogoMark, Icon, CategoryIcon, StarRating, VerifiedShield,
...
const Logo = ({ size = 32, showTagline = false, mono = false }) => (
...
// Generic stroke icon
const Icon = ({ name, size = 20, stroke = 'currentColor', strokeWidth = 1.7 }) => {
  const paths = {
...
const CategoryIcon = ({ name, size = 24 }) => {
  ...
  return <Icon name={map[name] || 'home'} size={size} strokeWidth={1.4} />;
};

const StarRating = ({ value = 4.9, size = 14, showNumber = true, count }) => (
  <div style={{display:'inline-flex', alignItems:'center', gap: 4, color: 'var(--ink)', fontSize: 13, fontWeight: 600}}>
    <span style={{color: '#111', display:'inline-flex'}}>
      <Icon name="starFilled" size={size} stroke="#111" />
    </span>
    {showNumber && <span>{value.toFixed(2)}</span>}
    {count != null && <span style={{color: 'var(--slate-500)', fontWeight: 500}}>({count})</span>}
  </div>
);

const VerifiedShield = ({ size = 18, label }) => (
  <span title="Verified user" style={{display:'inline-flex', alignItems:'center', gap: 6}}>
    <span className="verified-shield" style={{width: size, height: size}}>
      <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" style={{width: size * 0.6, height: size * 0.6}}>
        <path d="M5 12l4 4 10-10" />
      </svg>
    </span>
    {label && <span style={{fontSize: 12, fontWeight: 600, color: 'var(--gold-500)'}}>{label}</span>}
  </span>
);

const AVATAR_BG = ['#0B1F3A','#1AA8A1','#2D5BFF','#7C3AED','#0F766E','#0EA5E9','#9333EA','#DC2626','#0891B2','#65A30D'];
const Avatar = ({ name = '', size = 36, src, verified = false, ring = false }) => {
  ...
  return (
    <span style={{position:'relative', display:'inline-block', width: size, height: size}}>
      <span style={{
        ...
        background: src ? `center/cover url(${src})` : AVATAR_BG[idx],
        color: 'white', display:'flex', alignItems:'center', justifyContent:'center',
        fontWeight: 600, fontSize: size * 0.38, letterSpacing: '0.02em',
        boxShadow: ring ? '0 0 0 2px var(--paper), 0 0 0 4px var(--gold-400)' : 'none',
      }}>
        {!src && initials}
      </span>
      {verified && (
        <span style={{
          ...
          <VerifiedShield size={Math.max(14, size * 0.4)} />
        </span>
      )}
    </span>
  );
...
const PlaceholderImg = ({ label = 'image', height = 200, style = {} }) => (
  <div className="ph" style={{height, borderRadius: 'inherit', ...style}}>
    <span>{label}</span>
  </div>
);

Object.assign(window, { Logo, LogoMark, Icon, CategoryIcon, StarRating, VerifiedShield, Avatar, PlaceholderImg });
